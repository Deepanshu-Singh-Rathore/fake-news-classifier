...
def balance(df: pd.DataFrame, method: str, per_class: int | None = None) -> pd.DataFrame:
    ...
    if len(counts) < 2:
        return df
    target = per_class if per_class is not None else (counts.min() if method == "undersample" else counts.max())
    parts = []
...
def main():
    ...
    parser.add_argument("--balance", choices=["none", "undersample", "oversample"], default="none", help="Balance strategy across classes")
